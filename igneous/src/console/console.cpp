...
void Console::execute(call_sequence calls, bool positive)
{
	for (call c : calls)
	{
...
ConVar& Console::get(const std::string& variable)
{
...
void Console::run(const std::string& name, arg_list args)
{
	RETURN_INVALID(name);
...
void Console::runBind(int key, bool positive)
{
	if (bindExists(key))
	{
...
ConVar& Console::variable(const std::string& name, float defaultValue, convar_callback callback)
{
	return variables[name] = ConVar(name, defaultValue, callback);
}

ConVar& Console::variable(const std::string& name, float defaultValue, float initialValue, convar_callback callback)
{
